...
def evaluate(classifier, test_loader, ood_loader, train_features, train_labels, training_opt, device="cuda"):
    classifier.eval()
    test_preds = []
    test_labels = []
    test_features = []
    ...
    out_preds = torch.cat(out_preds, dim=0)
    out_labels = torch.cat(out_labels, dim=0)
    out_features = torch.cat(out_features, dim=0)

    #get the sigle vectors for each class
    first_sing_vecs, svd_values = get_svd_per_class_torch(train_features.to(device), train_labels.to(device), topN=training_opt['svd_por_1'])

    features_combined = torch.cat([test_features, out_features], dim=0)
    scores = score_func_svd_torch(features_combined, first_sing_vecs, split_N=training_opt['split_N'], top_N=training_opt['svd_por_2'])
    #scores = score_func_svd_weighted(features_combined, first_sing_vecs, svd_values, split_N=64, top_N=0.6)

    scores, _ = torch.min(scores, dim=1, keepdim=True)
    scores = 1 - scores
    print("test_scores_mean=", torch.mean(scores[0:test_preds.shape[0]]), "   ood_mean=", torch.mean(scores[test_preds.shape[0]:]))
    target_in = torch.zeros_like(test_labels)
    target_out = torch.ones_like(out_labels)
    targets = torch.cat((target_in, target_out), dim=0).detach().cpu().numpy()

    
    fpr, tpr, thresholds = metrics.roc_curve(targets, scores.detach().cpu().numpy())
    fpr95 = fpr[tpr >= 0.95][0]
    auc = metrics.auc(fpr, tpr)
    detection_error = np.min(0.5 * (1 - tpr) + 0.5 * fpr)

    test_acc = float(torch.sum(test_preds==test_labels))/test_preds.shape[0]
        ...
        for data in tqdm(data_loader['train']):
            ...
            if training_opt['mixed_loss']:
                ...
                loss = get_loss2(logits=logits_mixed2, 
                                y_a=y, y_b=y_mixed,
                                features=feat_mixed2, 
                                mixed_features=feat_mixed1,
            ...
            total_preds.append(pred.detach().clone().cpu())
            total_labels.append(y.detach().clone().cpu())
            total_features.append(feat_mixed2.detach().clone().cpu())
            

        total_preds = torch.cat(total_preds, dim=0)
        total_labels= torch.cat(total_labels, dim=0)
        total_features = torch.cat(total_features, dim=0)
        train_acc = float(torch.sum(total_preds==total_labels))/total_preds.shape[0]

        
        #evaluation
        
        if epoch>=training_opt['num_epochs']-3 :
            test_result = evaluate(classifier, data_loader['test'], data_loader['ood'], total_features, total_labels, training_opt)
...
if __name__=='__main__':
    ...
    parser.add_argument('--cfg', default='./config/CIFAR10/resnet.yaml', type=str)
