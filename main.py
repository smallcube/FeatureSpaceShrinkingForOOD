...
def get_loss(logits, logits_mixed, y_a, y_b=None, features=None, mixed_features=None, lam=1, base_weight=1, gamma=1, mixed_loss=True):
...
def train(config):
    ...
    for epoch in range(1, training_opt['num_epochs'] + 1):
        ...
        for data in tqdm(data_loader['train']):
            ...
            logits, feat = classifier(x, return_feat=True)
            logits_mixed, feat_mixed = classifier(x_mixed, return_feat=True)
            
            loss = get_loss(logits=logits, logits_mixed=logits_mixed, 
                            y_a=y, y_b=y_mixed, 
                            features=feat, mixed_features=feat_mixed,
                            lam=lam,
            ...
            total_features.append(torch2numpy(feat))
