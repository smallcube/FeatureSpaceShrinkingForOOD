...
def evaluate(classifier, test_loader, ood_loader, train_features, train_labels, device="cuda"):
    ...
    first_sing_vecs, svd_values = get_svd_unsupervised(train_features, topN=0.5)
    
    features_combined = np.concatenate([test_features, out_features], axis=0)
    scores = score_func_svd_unsupervised(features_combined, first_sing_vecs, split_N=1, top_N=0.5)
...
def train(config):
    ...
    for epoch in range(1, training_opt['num_epochs'] + 1):
        ...
        #evaluate(classifier, data_loader['test'], data_loader['ood'], total_features, total_labels)
